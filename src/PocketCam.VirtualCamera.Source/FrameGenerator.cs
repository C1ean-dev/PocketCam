...
public class FrameGenerator : IDisposable
{
    ...
    public IComObject<IMFSample> Generate(IComObject<IMFSample> sample, Guid format)
    {
        try
        {
            ...
            // The sample's diagnostic pattern is useful while no desktop frame exists.
            // As soon as PocketCam publishes a frame, overwrite the WIC surface with it.
            if (!HasD3DManager && _bitmap != null)
            {
                var pixels = _frameReader.ReadScaledBgra((int)_width, (int)_height);
                using var writable = _bitmap.Lock(WICBitmapLockFlags.WICBitmapLockWrite);
                writable.Object.GetDataPointer(out var writableSize, out var writablePointer).ThrowOnError();
                Marshal.Copy(pixels, 0, writablePointer, Math.Min(pixels.Length, checked((int)writableSize)));
            }
